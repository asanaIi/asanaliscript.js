...
<body>
    ...
    startWebServer(port = 3000, defaultHtml = "") {
        ...
        server.listen(port, () => {
            console.log(`\n[aScript Web Server] 🚀 Сервер сайта работает на http://localhost:${port}`);
        });
    }
    ...
    runFile(filename) {
        ...
        while (this.currentLine < this.lines.length) {
            ...
            // Обработка фреймворк блока html = { }
            ...
            // Графика окон window()
                ...
                rawPath = cleanLine.slice(5).trim();
                resolvedPath = this.replaceVariables(rawPath);
...
if (require.main === module) {
    ...
    if (process.argv.length > 2) {
        ...
        if (!found) {
            console.log("AsanaliScript (aScript) v2.1-TS Web Engine");
